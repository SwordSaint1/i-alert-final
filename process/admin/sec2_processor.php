...
if ($method == 'fetch_sec2') {
        ...
        $sec2 = "SELECT * FROM ialert_audit WHERE section = 'Section2' AND pd IS NULL AND agency IS NULL AND employee_num LIKE '$empid%' AND full_name LIKE '$fname%' AND line_no LIKE '$line%' AND car_maker LIKE '$carmaker%' AND car_model LIKE '$carmodel%' AND position LIKE '$position%' AND provider = 'FAS'";
...
if ($method == 'count_section2') {
	...
	foreach($stmt->fetchALL() as $x){
        ...
        $days_notif = date("Y-m-d", strtotime('+4 day',strtotime($date_audited)));
        	...
        	$count_na = "SELECT COUNT(*) as total FROM ialert_audit WHERE  pd IS NULL AND agency IS NULL AND section = 'Section2' AND (date_audited >='$dateFrom' AND date_audited <= '$dateTo') AND provider = 'FAS'";
...
if ($method == 'total_sec2') {
	$dateFrom = $_POST['dateFrom'];
	$dateTo = $_POST['dateTo'];
	$count_total = "SELECT count(*) as grand_total from ialert_audit where section = 'Section2' AND (date_audited >='$dateFrom' AND date_audited <= '$dateTo') AND provider = 'FAS'";
